...
with mlflow.start_run(run_name="Random Forest Tuning") as parent_run:
    ...
    signature  = infer_signature = infer_signature(X_test, search.best_estimator_.predict(X_test))
    

    mlflow.sklearn.log_model(search.best_estimator_, "Best Model",signature=signature)
